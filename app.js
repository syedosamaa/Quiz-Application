
// ? HTML Questions //  

function htmlQuest() {

  let htmlQuestions = [

    {
      num: 1,
      question: "HTML stands for",
      Option: {
        a: "Hyper Text Markup Language",
        b: "Hyper Text Programming Language",
        c: "Hyper Tool Multi Language",
        d: "Hyperlinks and Text Markup Language"
      },
      answer: "Hyper Text Markup Language"
    },
    {
      num: 2,
      question: "Which tag is used to define a paragraph in HTML?",
      Option: {
        a: "<p>",
        b: "<para>",
        c: "<par>",
        d: "<pg>"
      },
      answer: "<p>"
    },
    {
      num: 3,
      question: "Which tag is used to display an image in HTML?",
      Option: {
        a: "<image>",
        b: "<img>",
        c: "<src>",
        d: "<pic>"
      },
      answer: "<img>"
    },
    {
      num: 4,
      question: "What is the correct HTML tag for inserting a line break?",
      Option: {
        a: "<br>",
        b: "<lb>",
        c: "<break>",
        d: "<newline>"
      },
      answer: "<br>"
    },
    {
      num: 5,
      question: "Which tag is used to create a hyperlink?",
      Option: {
        a: "<a>",
        b: "<link>",
        c: "<href>",
        d: "<url>"
      },
      answer: "<a>"
    },
    {
      num: 6,
      question: "Which attribute is used to specify the destination of a link?",
      Option: {
        a: "src",
        b: "link",
        c: "href",
        d: "target"
      },
      answer: "href"
    },
    {
      num: 7,
      question: "Which tag is used to make text bold in HTML?",
      Option: {
        a: "<strong>",
        b: "<bold>",
        c: "<bld>",
        d: "<black>"
      },
      answer: "<strong>"
    },
    {
      num: 8,
      question: "Which tag is used to define the largest heading in HTML?",
      Option: {
        a: "<h1>",
        b: "<heading>",
        c: "<head>",
        d: "<h6>"
      },
      answer: "<h1>"
    },
    {
      num: 9,
      question: "Which tag is used to define a table row?",
      Option: {
        a: "<td>",
        b: "<tr>",
        c: "<th>",
        d: "<table>"
      },
      answer: "<tr>"
    },
    {
      num: 10,
      question: "What is the correct tag to add a comment in HTML?",
      Option: {
        a: "// Comment",
        b: "/* Comment */",
        c: "<!-- Comment -->",
        d: "# Comment"
      },
      answer: "<!-- Comment -->"
    }
  ]

  return htmlQuestions;
}


//? CSS Questions //

function cssQuest() {

  let cssQuestions = [

    {
      num: 1,
      question: "CSS stands for",
      Option: {
        a: "Cascading Style Sheets",
        b: "Creative Style System",
        c: "Computer Styling Sheets",
        d: "Colorful Style Syntax"
      },
      answer: "Cascading Style Sheets"
    },
    {
      num: 2,
      question: "Which HTML tag is used to link an external CSS file?",
      Option: {
        a: "<style>",
        b: "<link>",
        c: "<css>",
        d: "<script>"
      },
      answer: "<link>"
    },
    {
      num: 3,
      question: "Which property is used to change the text color in CSS?",
      Option: {
        a: "font-color",
        b: "color",
        c: "text-color",
        d: "font-style"
      },
      answer: "color"
    },
    {
      num: 4,
      question: "Which property controls the size of text?",
      Option: {
        a: "font-size",
        b: "text-size",
        c: "font-style",
        d: "text-font"
      },
      answer: "font-size"
    },
    {
      num: 5,
      question: "Which property is used to change the background color?",
      Option: {
        a: "color",
        b: "background",
        c: "background-color",
        d: "bgcolor"
      },
      answer: "background-color"
    },
    {
      num: 6,
      question: "Which CSS property is used to make text bold?",
      Option: {
        a: "text-bold",
        b: "font-bold",
        c: "font-weight",
        d: "bold"
      },
      answer: "font-weight"
    },
    {
      num: 7,
      question: "How can you center text inside an element?",
      Option: {
        a: "align: center",
        b: "text-align: center",
        c: "font-align: center",
        d: "margin: auto"
      },
      answer: "text-align: center"
    },
    {
      num: 8,
      question: "Which property is used to change the font of text?",
      Option: {
        a: "font-family",
        b: "font-type",
        c: "font-style",
        d: "text-font"
      },
      answer: "font-family"
    },
    {
      num: 9,
      question: "Which property is used to add space inside an element?",
      Option: {
        a: "margin",
        b: "padding",
        c: "border",
        d: "spacing"
      },
      answer: "padding"
    },
    {
      num: 10,
      question: "Which symbol is used to select a class in CSS?",
      Option: {
        a: "#",
        b: ".",
        c: "$",
        d: "@"
      },
      answer: "."
    }
  ]

  return cssQuestions;
}

//? JavaScript Questions //

function jsQuest() {

  let jsQuestions = [
    {
      num: 1,
      question: "JavaScript is a ___ language",
      Option: {
        a: "Programming",
        b: "Markup",
        c: "Styling",
        d: "Database"
      },
      answer: "Programming"
    },
    {
      num: 2,
      question: "Which symbol is used for comments in JavaScript?",
      Option: {
        a: "//",
        b: "<!-- -->",
        c: "**",
        d: "##"
      },
      answer: "//"
    },
    {
      num: 3,
      question: "Which keyword is used to declare a variable in JavaScript?",
      Option: {
        a: "var",
        b: "int",
        c: "string",
        d: "declare"
      },
      answer: "var"
    },
    {
      num: 4,
      question: "Which method is used to print something on the console?",
      Option: {
        a: "print()",
        b: "log()",
        c: "console.log()",
        d: "document.write()"
      },
      answer: "console.log()"
    },
    {
      num: 5,
      question: "Which operator is used to assign a value to a variable?",
      Option: {
        a: "-",
        b: "=",
        c: "*",
        d: "=="
      },
      answer: "="
    },
    {
      num: 6,
      question: "What will 'typeof 42' return?",
      Option: {
        a: "number",
        b: "string",
        c: "boolean",
        d: "object"
      },
      answer: "number"
    },
    {
      num: 7,
      question: "Which function is used to display an alert box?",
      Option: {
        a: "alert()",
        b: "prompt()",
        c: "confirm()",
        d: "show()"
      },
      answer: "alert()"
    },
    {
      num: 8,
      question: "Which method joins array elements into a single string?",
      Option: {
        a: "concat()",
        b: "join()",
        c: "merge()",
        d: "combine()"
      },
      answer: "join()"
    },
    {
      num: 9,
      question: "Which keyword is used to define a constant variable?",
      Option: {
        a: "var",
        b: "let",
        c: "const",
        d: "static"
      },
      answer: "const"
    },
    {
      num: 10,
      question: "What does 'NaN' stand for in JavaScript?",
      Option: {
        a: "Not a Number",
        b: "No any Number",
        c: "Notable Numeric",
        d: "Null as Number"
      },
      answer: "Not a Number"
    }
  ]


  return jsQuestions;
}


// ? Get all pages //

let userDetails = document.querySelector('.form-wrapper');
let startScreen = document.querySelector('.start-screen');
let quizPage = document.querySelector('.quiz-page');

//? Get all input fields //

let inpFName = document.getElementById('first-name');
let inpLName = document.getElementById('last-name');
let inpEmail = document.getElementById('email');
let inpRoll = document.getElementById('roll-no');

//? Get Quesstions//

let toltalQuest = document.querySelector('.ttl-questions');
let questionText = document.getElementById('question');
let optionList = document.getElementById('options').getElementsByTagName('li');

let currentQuiz = [];
let currentIndex = 0;
let correctAns = 0;
let wrongAns = 0;
let missedAns = 0;

let nxtBtn = document.querySelector('.nextBtn');


//? Function for start quiz //

function formValidate() {


  let selectedQuiz = document.querySelector('input[name="quiz-type"]:checked');

  if (!inpFName.value || !inpLName.value || !inpEmail.value || !inpRoll.value) {
    Swal.fire({
      icon: 'warning',
      title: 'Incomplete Form',
      text: 'Please fill all fields and select a quiz.',
      confirmButtonColor: "#3498db",
    });
    return false

  }

  if (!inpEmail.value.includes('@')) {
    Swal.fire({
      icon: 'error',
      title: 'Invalid Email',
      text: 'Please enter a valid email address.',
      confirmButtonColor: "#3498db",
    })
    return false;

  }


  userDetails.style.display = "none";
  startScreen.style.display = "flex";



  //? User Details get and set //


  document.getElementById('para-name').innerText = `${inpFName.value} ${inpLName.value}`;
  document.getElementById('para-email').innerText = inpEmail.value;
  document.getElementById('para-roll-no').innerText = inpRoll.value;
  document.getElementById('para-quiz-select').innerText = selectedQuiz.value;

}


function startQuiz() {
  userDetails.style.display = "none"
  startScreen.style.display = "none";
  quizPage.style.display = "flex";


  let selectedQuiz = document.querySelector('input[name="quiz-type"]:checked');

  let questions = 0;

  if (selectedQuiz.value === "HTML") {
    questions = htmlQuest();
  }
  else if (selectedQuiz.value === "CSS") {
    questions = cssQuest();
  }
  else if (selectedQuiz.value === "JavaScript") {
    questions = jsQuest();
  }

  currentQuiz = questions;

  document.querySelector(".ttl-questions").innerText = questions.length;

  showQuestion(questions, 0);
}

function showQuestion(questions, index) {

  let q = questions[index];


  questionText.innerText = q.question;



  let Opts = q.Option;

  optionList[0].innerText = "a . " + Opts.a;
  optionList[1].innerText = "b . " + Opts.b;
  optionList[2].innerText = "c . " + Opts.c;
  optionList[3].innerText = "d . " + Opts.d;

  document.querySelector(".q-nmbr").innerText = q.num;

  for (let li of optionList) {
    li.classList.remove("correctAns", "wrongAns", "disableli");
    li.setAttribute("onclick", "selectOpt(this)");
  }

  nxtBtn.style.display = "none";

  startTimer();

}

function selectOpt(ele) {

  let currentQ = currentQuiz[currentIndex];
  let allOptions = document.querySelectorAll("#options li");

  if (ele.innerText.includes(currentQ.answer)) {

    ele.classList.add("correctAns");
    correctAns++;
  } else {
    ele.classList.add("wrongAns")
    wrongAns++;

    for (let li of allOptions) {
      if (li.innerText.includes(currentQ.answer)) {
        li.classList.add("correctAns");
      }
    }
  }

  for (let li of allOptions) {
    li.classList.add("disableli");
  }

  document.querySelector(".nextBtn").style.display = "block";

  stopTimer()

}

function nextQuestion() {
  currentIndex++;
  if (currentIndex < currentQuiz.length) {
    showQuestion(currentQuiz, currentIndex);
  } else {
    Swal.fire({
      icon: 'success',
      title: 'Quiz Complete!',
      text: `Correct: ${correctAns}, Wrong: ${wrongAns} , Missed : ${missedAns}`,
      confirmButtonColor: "#3498db",
    }).then(function () {
      quizResult();
    })
  }


}

let timer;
let timeLeft = 20;

function startTimer() {
  clearInterval(timer);
  timeLeft = 20;
  document.getElementById('time-count').innerText = timeLeft + 's';

  timer = setInterval(function () {
    timeLeft--;
    document.getElementById('time-count').innerText = timeLeft + 's';

    if (timeLeft <= 0) {
      clearInterval(timer);
      autoNextQuestion();
    }

  }, 1000);


}


function autoNextQuestion() {
  let currentQ = currentQuiz[currentIndex];
  let allOptions = document.querySelectorAll("#options li");


  for (let li of allOptions) {
    li.classList.add("disableli");
    if (li.innerText.includes(currentQ.answer)) {
      li.classList.add("correctAns");
    }
  }

  missedAns++;


  setTimeout(function () {
    currentIndex++;
    if (currentIndex < currentQuiz.length) {
      showQuestion(currentQuiz, currentIndex);
    }
    else {
      Swal.fire({
        icon: 'success',
        title: 'Quiz Complete!',
        text: `Correct: ${correctAns}, Wrong: ${wrongAns}`,
        confirmButtonColor: "#3498db",
      }).then(function () {
        quizResult();
      })
    }

  }, 1000);
}

function stopTimer() {
  clearInterval(timer);
}


//? Get result page & details //

let resultPage = document.querySelector('.result-page');

let resultName = document.querySelector('#result-name');
let resultEmail = document.querySelector('#result-email');
let resultRoll = document.querySelector('#result-roll');
let resultQuiz = document.querySelector('#result-quiz');

//? Get  stats //

let ttlQuest = document.querySelector('.ttl-questions');
let ttlquest = document.querySelector('#ttl-quest');
let right = document.querySelector('#rightAns');
let wrong = document.querySelector('#wrongAns');
let missed = document.querySelector('#missedAns');
let percent = document.querySelector('#percentile');
let paraContent = document.querySelector('.para-content');


function quizResult() {

  quizPage.style.display = "none";
  resultPage.style.display = "flex";

  let selectedQuiz = document.querySelector('input[name="quiz-type"]:checked');
  paraContent = document.querySelector('.para-content');

  paraContent.classList.remove()

  resultName.innerHTML = `${inpFName.value} ${inpLName.value}`;
  resultEmail.innerHTML = inpEmail.value;
  resultRoll.innerHTML = inpRoll.value;
  resultQuiz.innerHTML = selectedQuiz.value;

  ttlquest.innerHTML = currentQuiz.length;
  right.innerHTML = correctAns;
  wrong.innerHTML = wrongAns;
  missed.innerHTML = missedAns;

  let percentage = Math.round((correctAns / currentQuiz.length) * 100);
  percent.innerHTML = percentage;


  if (percentage < 60) {
    paraContent.innerHTML = "Sorry, you failed! Try Again!";

    paraContent.classList.add("fail-para");
  } else {
    paraContent.innerHTML = "Congo, You Passed!";

    paraContent.classList.add("pass-para");
  }

};


